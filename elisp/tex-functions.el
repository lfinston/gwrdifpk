...
;; Added `insert-single_quotes'.  LDF 2004.03.17.
...
(defun bold-face (arg)
  "bold-face inserts {\\bf } and prompts for a string to put inside.
It works for tex and el-tex files and inserts \\\\ for the latter.
If the user types <RET> without entering a string, it leaves point between the 
delimiters. Otherwise, it leaves point outside the delimiters.
It functions differently depending on the prefix argument.
...
delimiters and prompts the user to cycle through the kill ring or exit."
  (interactive "P")
  (let (bold-face-string)
    (if (or (string= mode-name "Emacs-Lisp") (string= mode-name "Lisp"))
	(setq bold-face-string "{\\\\bf ")
      ;; else
      (setq bold-face-string "{\\bf "))
    (tgdf bold-face-string "}"
				    "What should be in bold face? " arg)
    ;; Matches let
    )
  ;; Matches bold-face
  )
...
(defun texi2dvi-file-ldf (arg)
  "texi2dvi-file-ldf runs texi2dvi on a file. 
...
Currently, if arg is nil, tex is run instead of texi2dvi. This prevents TeX from being
called twice, if you're not interested in having the cross-references be
handled correctly. Normally, if arg is nil, texi2dvi will be run."
  (interactive "P")
  (let ((this-buffer (get-buffer (current-buffer)))
    ...
    (setq example-file-old (concat current-dir "examples.web"))
    (setq example-file-new (concat current-dir "../" "examples.web"))
    (setq extext-file-old (concat current-dir "extext.tex"))
    (setq extext-file-new (concat current-dir "../" "extext.tex"))
    (save-buffer)
    (save-some-buffers t)
    (delete-other-windows)
    (make-comint "texinfo-shell" "/bin/ksh" nil "-i")
    ...
    ;; (insert ";mv examples.web ..; mv extext.tex ..")
    (comint-send-input)
    (switch-to-buffer-other-window this-buffer)
    (sit-for 1.5)
    (message nil)
...
delimiters and prompts the user to cycle through the kill ring or exit."
  (interactive "P")
  (let (s)
    (setq s (concat (char-to-string 92) (char-to-string 167)))
